  ...
  const handleAnalyze = async (repo) => {
    ...
    try {
      ...
      if (err.response?.status === 404) {
        msg = `Repository "${repo}" not found. Please check the owner/repo format.`;
      } else if (err.response?.status === 403) {
        msg = "GitHub API rate limit hit. Please wait a moment and try again.";
      } else if (err.code === "ERR_NETWORK" || err.message === "Network Error") {
        msg = "Cannot reach backend. Make sure it's running on http://localhost:5000";
      }
      setError(msg);
    } finally {
      setIsLoading(false);
      ...
      <main>
        <div className="container">
          ...
          <section className="hero">
            <div className="hero-tag">✦ Powered by GitHub API</div>
            <h1 className="hero-title">
              Analyse Open Source<br />
              <span className="gradient-text">Issues Intelligently</span>
            </h1>
            <p className="hero-subtitle">
              Discover beginner-friendly issues, detect duplicate reports, measure staleness,
              and get difficulty ratings — all in one glance.
            </p>
              ...
              {filteredIssues.length > 0 ? (
                ...
                <div className="empty-state">
                  ...
                  <p className="empty-desc">
                    Try adjusting the filters or search query above.
                  </p>
                </div>
              )}

              {/* Duplicates + First Timer Info */}
          ...
          {!isLoading && !hasAnalyzed && !error && (
            <div className="empty-state" style={{ paddingTop: 40 }}>
              ...
              <p className="empty-desc">
                Enter any public GitHub repository above to get a full breakdown of open issues
                — difficulty, staleness, first-timer opportunities & more.
              </p>
            </div>
          )}
        </div>
